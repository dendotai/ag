...
about the project. Without the file it stops and names it.
...
export default defineConfig({
  adapter: "convex",
  convex: { apiDir: "packages/api", team: "my-team", project: "my-project" },
  // Values a fresh environment needs before its first push. A value already
  // stored on the environment is never touched, so a rerun changes nothing
  // and a value set by hand survives.
  env: ({ secret }) => ({ AUTH_SECRET: secret() }),
  // Env files to write once the environment exists, keyed by path relative to
...
environment that exists and replaces one that is gone.
...
from the OS when it starts, and tells its backend its address at that
moment, if the backend needs one.

The **adapter** named in the config turns name and lifetime into an
environment for the project's stack.
...
the credential; nothing is stored. Create passes the lifetime as the
deployment's expiration. Then the adapter stores the config's `env` values
the deployment lacks, pushes once with `convex dev --once`, and writes the
config's `files` with the deployment's URL.
